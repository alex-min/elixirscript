defmodule ElixirScript.Translate.Forms.Bitstring do
  ...
  def compile_element({:"::", _, [element, {type, _, _}]}, state)
      when type in [
             :integer,
             :float,
             :bitstring,
  ...
  def compile_element({:"::", _, [element, {type, _, params}]}, state)
      when type in [:size, :unit] do
    do_compile_element(
      {type, translate_element(element, state), Enum.map(params, &translate_element(&1, state))}
    )
  end
  ...
  def compile_element({:"::", _, [element, {:*, _, [size, unit]}]}, state) do
    size_ast =
      do_compile_element(
        {:size, translate_element(element, state), [translate_element(size, state)]}
      )
