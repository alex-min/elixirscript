defmodule ElixirScript.Translate.Forms.With do
  ...
  def compile(args, module_state) do
    result =
      Enum.reduce(args, %{expressions: [], arguments: [], module_state: module_state}, fn
        {symbol, _, [pattern, body]}, state when symbol in [:<-, :=] ->
          ...
          %{
            state
            | arguments: state.arguments ++ params,
              expressions:
                state.expressions ++ [JS.array_expression([hd(patterns), expr_function])],
